...
|---|---|---|---|---|---|
| 1 | Insecure config: `DEBUG=True` (source code) | `app/main.py:27` | `config_auditor.py` | Tier 1 (auto-fix) | 0.90 |
| 1b | Insecure config: wildcard CORS | `app/main.py:31` | `config_auditor.py` | Tier 1 (auto-fix) | 0.92 |
| 2 | Hardcoded secret (synthetic Stripe-shaped demo key) | `app/main.py:43` | `secret_scanner.py` | Tier 1 (auto-fix) | 0.98 |
| 3 | Hardcoded auth-bypass conditional | `app/main.py:71`, `verify_admin()` | `ast_rules.py` | Tier 2 (approve-and-fix) | 0.84 |
| 4 | Obfuscated backdoor: base64-decoded payload executed via `subprocess.run(..., shell=True)` | `app/main.py:99`, `health_check()` | `ast_rules.py` | Tier 2 (approve-and-fix) | 0.95 |
...
| 6b | `DEBUG=True` inside the committed `.env` | `.env:4` | `config_auditor.py` | Tier 1 (auto-fix) | 0.90 |
...
| 8 | Prompt injection: unsanitized user input concatenated directly into an LLM prompt | `app/main.py:133`, `summarize_task()` | `prompt_injection_rules.py` | Tier 3 (flagged only, no auto-fix — review recommended) | ~0.62 |
| 9 | Unsafe model deserialization: `pickle.load()` on an attacker-controllable file path | `app/main.py:152`, `load_model()` | `model_security_rules.py` | Tier 2 (approve-and-fix) | ~0.94 |
